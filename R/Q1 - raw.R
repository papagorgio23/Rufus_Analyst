...
write.csv(nfl_schedule, "nfl2019.csv", row.names = FALSE)
## load functions
...
#' new_datanfl <- scrape_game_ids(2018) 
#' @export
season = 2018
...
testing <- colnames(pass_offense)
testing[19] <- "pass_off_sack_yds"
...
convertTeamAbbreviation <- function(x){
  ...
  # x[grep("LA", x, ignore.case=TRUE)] <- "LAR"
...
# change to numeric data
all_data[, 2:length(all_data)] <- sapply(all_data[, 2:length(all_data)], cleanPercent)
all_data[, 2:length(all_data)] <- sapply(all_data[, 2:length(all_data)], as.numeric)
...
nfl_weeks <- nfl_weeks %>% 
  mutate(home_team = convertTeamAbbreviation(home_team),
         away_team = convertTeamAbbreviation(away_team))
...
# fix team names
all_data$Tm <- convertTeamAbbreviation(all_data$Tm)

# split for home and away teams
home_stats <- all_data
away_stats <- all_data

# rename ranking data for home and away teams
names <- colnames(all_data)
home_names <- paste0("Home_", names)
away_names <- paste0("Away_", names)
colnames(home_stats) <- home_names
colnames(away_stats) <- away_names


## Join week 16 games with rankings
nfl_week16 <- nfl_week16 %>% 
  left_join(home_stats, by = c("home_team" = "Home_Tm")) %>%
  left_join(away_stats, by = c("away_team" = "Away_Tm"))


#### SIMPLE RATING SYSTEM
SRS <- nfl_week16 %>%
  select(game_id, week, home_team, away_team, Home_SRS, Away_SRS) %>% 
  mutate(Home_SRS = as.numeric(Home_SRS),
         Away_SRS = as.numeric(Away_SRS),
         neutral_spread = Away_SRS - Home_SRS,
         home_spread = Away_SRS - Home_SRS - 2) # add roughly 2 points for home team




#### Basic model for each team...
# Each team gets 3 inputs... 
# Passing
# Rushing
# Special Teams
# Pace
...
### MONTE CARLO SIM GAME


# number of drives
# success rate of drive
# if successful is it TD FG 
str(all_data)
str(kicking)

# get only useful stats for the simulation
monte <- all_data %>% 
  select(Tm, `tm_off_Sc%`, `tm_off_TO%`, tm_off_Passing_TD, tm_off_Rushing_TD, `drive_#Dr`, kick_Scoring_FGM) %>% 
  mutate(total_TD = tm_off_Passing_TD + tm_off_Rushing_TD,
         Punt_perc = 100 - `tm_off_Sc%` - `tm_off_TO%`,
         TD_ratio = round(total_TD / (total_TD + kick_Scoring_FGM), 2),
         FG_ratio = round(kick_Scoring_FGM / (total_TD + kick_Scoring_FGM), 2),
...
sim_game(home_team = "SF", away_team = "LAR")
...
for (j in 1:nrow(nfl_weeks)) {
  ...
  print(paste0("  --  Finished with 10,000 simulations of ", home, " vs ", away, "!"))
}
# end progress bar
close(pb)
...
sim_results_team <- sim_results %>% 
  ...
  summarise(home_team = last(home_team),
            ...
            ties = sum(Tie),
            home_win_prob = sum(home_wins) / n(),
            away_win_prob = sum(away_wins) / n(),
            tie_prob = ties / n(),
            home_score_med = median(home_score),
            away_score_med = median(away_score),
            home_score_avg = mean(home_score),
            away_score_avg = mean(away_score),
            home_score_sd = sd(home_score),
            away_score_sd = sd(away_score),
            home_line_med = median(Home_line),
            home_line_avg = mean(Home_line),
            home_line_sd = sd(Home_line)) %>% 
  select(1:3, 7:9, everything())

sim_results_team <- sim_results_team %>% 
...
str(DF)
DF <- data.frame((round(table(sim_results$home_score, sim_results$away_score)/num_sims, 4))*100)
d3heatmap(DF, scale = "column", dendrogram = "row", k_row = 3, colors = "Blues")

## Home Score vs Away Score
(round(table(sim_results$home_score, sim_results$away_score)/num_sims, 4))*100

sim_plot <- (round(table(sim_results$home_score, sim_results$away_score)/num_sims, 4))*100


library(d3heatmap)
# interactive heatmap with 3 clusters
d3heatmap(sim_plot, scale = "column", dendrogram = "row", k_row = 3, colors = "Blues")

sim_plot <- matrix(sim_plot)


# correlation plot
corrplot(sim_plot, col = colorRampPalette(c("royalblue", "white", "orangered2"))(200))

?corrplot
mean(sim_results$Home_line)

(sum(sim_results$Home_line < -2.5))/nrow(sim_results)
(sum(sim_results$Home_line < -3.5))/nrow(sim_results)

sim_results$week
# plot score differential
sim_results %>% 
  mutate(label = paste0("Week ", week, ":  ", away_team, "  @  ", home_team)) %>% 
  ggplot(aes(x = Home_line)) +
  geom_histogram(binwidth = 2, color = "blue", fill = "skyblue") +
  facet_wrap(~label) + 
  theme_bw()
...
theme_538 <- function(base_size = 12, font = "Impact") {
  ...
  large_txt <- element_text(size = base_size + 4, color = "black", face = "bold", family = "Impact")
...
sim_plot_data <- sim_results %>% 
  left_join(sim_results_team, by = c("home_team", "away_team", "game_id")) %>% 
...
library(patchwork)
twoplots <- c(game_plot1, game_plot)

str(sim_plot_data)

# Display the grid of plots with cowplot!
library(cowplot)
plot_grid(plotlist = twoplots, ncol = 2)
plot_grid(game_plot1, game_plot, ncol = 1)
?plot_grid
...
sim_results %>% 
  ...
  geom_density(aes(x = away_score),  color = "red", fill = "red", alpha = 0.5) +
  geom_vline(data = sim_results_team, xintercept = median(sim_results_team$home_score_avg), color = "blue") +
  geom_vline(xintercept = median(away_score), color = "red") +
  labs(x = "Score") +
  theme_bw()
...
# correlation plot
corrplot(cor_mat, order = "hclust", type = "upper", sig.level = 0.05, tl.cex = 0.35,
         tl.col="red", tl.srt=45, #Text label color and rotation
         col = colorRampPalette(c("royalblue", "white", "orangered2"))(200))

# correlation plot
corrplot(cor_mat, order = "hclust", sig.level = 0.05, tl.cex = 0.35,
         tl.col = "red", tl.srt = 45, #Text label color and rotation
         col = colorRampPalette(c("royalblue", "white", "orangered2"))(200))
...
sim_table <- table(sim_results$home_score, sim_results$away_score)
sim_prob <- prop.table(sim_table)
...
games1_plots <- lapply(games1,
                       function(game_label) {
                         ...
                         if (home_avg >= away_avg) {
                           new_plot %>% 
                             ggplot(aes(x = home_score)) +
                             geom_histogram(aes(color = last(home_primary), fill = last(home_primary)), binwidth = 1, alpha = 0.8) + 
                             scale_color_identity() +
                             scale_fill_identity() +
                             geom_histogram(aes(x = away_score , color = last(away_primary), fill = last(away_primary)), binwidth = 1, alpha = 0.8) + 
                             geom_vline(aes(xintercept = median(away_score_avg), color = last(away_primary))) + 
                             geom_vline(aes(xintercept = median(home_score_avg), color = last(home_primary))) + 
                             annotate("label", x = home_avg + 0.5, y = 1350, hjust = 0, vjust = 1 , size = 4, family = "Impact",
                                      label = paste0(home_team, ": ", home_avg)) +
                             annotate("label", x = away_avg - 0.5, y = 1350, hjust = 1, vjust = 1 , size = 4, family = "Impact",
                                      label = paste0(away_team, ": ", away_avg)) +
                             annotate("label", x = 38, y = 1350, hjust = 0, vjust = 1 , size = 5, fontface = "bold", family = "Impact",
                                      label = paste0("Home Line: ", game_line, "\nHome Win Probability: ", home_win*100, "%")) +
                             labs(x = "Score",
                                  y = "Count") +
                             facet_wrap(~label) +
                             theme_538() +
                           ...
                           new_plot %>% 
                             ...
                             geom_vline(aes(xintercept = median(away_score_avg), color = last(away_primary))) + 
                             geom_vline(aes(xintercept = median(home_score_avg), color = last(home_primary))) + 
                             annotate("label", x = home_avg - 0.5, y = 1350, hjust = 1, vjust = 1 , size = 4, family = "Impact",
                                      label = paste0(home_team, ": ", home_avg)) +
                             annotate("label", x = away_avg + 0.5, y = 1350, hjust = 0, vjust = 1 , size = 4, family = "Impact",
                                      label = paste0(away_team, ": ", away_avg)) +  
                             annotate("label", x = 38, y = 1350, hjust = 0, vjust = 1 , size = 5, fontface = "bold", family = "Impact",
                                      label = paste0("Home Line: ", game_line, "\nHome Win Probability: ", home_win*100, "%")) +
                             labs(x = "Score",
